...
gps\footnote{Debido al extendido uso de la denominación \textit{GPS} como sinónimo de los sistemas de posicionamiento por satélite, se usará de este modo. Para referirse al sistema de posicionamiento de los Estados Unidos \acf{GPS} se utilizará el acrónimo en mayúsculas} 
...
%HISTORIA DEL GPS
Los gps permiten conocer en tiempo real la posición de un objeto cualquiera en la superficie terrestre.
...
%HISTORIA DEL SIG
Debido a que el término \ac{SIG} engloba la integración de muy diversas áreas,no existe una única definición totalmente consensuada \cite{exploring_GIS}. La definición aportada por el \ac{NCGIA} resulta ampliamente aceptada:
\begin{tikzpicture}
	\node[shadowBox] {
...
Gracias a ello y referenciando en el mapa la posición de los pozos de agua, pudo comprobar como una gran cantidad de víctimas se encontraban dentro de la zona de influencia de una bomba de agua en Broad Street (ver figura \ref{fig:cholera_map_detail}), que a la postre resultó estar contaminada con heces. Recomendando la clausura de la misma consiguió acabar con la epidemia \cite{john_snow02}. Debido a estos logros se le considera el padre de la epidemiología moderna y podemos ilustrar uno de los primeros ejemplos del uso de los \ac{SIG}.
